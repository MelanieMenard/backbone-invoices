...
var InvoiceApp = Backbone.Marionette.Application.extend({
...
/* --- Initialisation code: init modules and pass them the data they need  --- */
  ...
  initialize: function(config) {
    ...
    // top level controller responsible for getting the data from server
    this.dataManager = new InvoiceApp.DataManager({
      appData: this.appData
    });

    // app top level controller and router
    this.routerController = new InvoiceApp.RouterController({
      appData: this.appData
    });

    this.router = new InvoiceApp.Router({
  ...
  initAppData: function(config) {
    ...
    this.appData.endpoints.invoice = config.baseUrl + config.invoiceEndpoint;
...
/* --- Code waiting for messages from data manager module  --- */
  ...
  onDataFetched: function() {
    ...
    // otherwise show the invoices page by default
    else {
      this.router.navigate('invoices', {trigger: true});
    }
  ...
  navigateToPage: function(routeBase, routeParam) {
    ...
    // test for undefined to allow routeParam to be 0, as the first invoice has Id 0
...
// Copy the errorHandler mixin methods to InvoiceApp app
_.extend(InvoiceApp.prototype, ErrorHandlerMixin);
